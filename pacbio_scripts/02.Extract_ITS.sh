...
find ./02_PrimerCut/data -name "*_trimmed_Full.fq.gz" | rush -j "$NCORES" \
...
find ./02_PrimerCut/data -name "*_trimmed_Part.fq.gz" | rush -j "$NCORES" \
  "seqkit replace -p '\s.+' -r ';sample={%:}' {} -o ./Primer_trim/{%:}_renamed.fq.gz"
...
itsxpress \
  --fastq GL_trimmed_Full_Merg.fq.gz \
  --cluster_id 1.0 \
  --single_end \
  --region "ALL" \
  --taxa "All" \
